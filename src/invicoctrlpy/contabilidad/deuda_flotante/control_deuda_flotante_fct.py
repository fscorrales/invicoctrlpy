...
# --------------------------------------------------
def rdeu012_with_accounting(
    ejercicios: [str] = None, db_path: str = None
) -> ControlDeudaFlotante:
    if not ejercicios:
    ...
    ejercicios = sorted(
        [str(ejercicio) for ejercicio in ejercicios], key=lambda x: int(x)
    )
    ...
    for ejercicio in ejercicios:
        ...
        rcocc31 = import_siif_rcocc31_liabilities(
            ejercicio=ejercicio, import_df=import_df
        )
        filter_rcocc31 = rcocc31_in_rdue012(
            ejercicio=ejercicio, rdeu=rdeu, rcocc31=rcocc31
        )
        ...
        aju = aju_not_in_rdue012(
            filter_rdeu=rdeu, rcocc31=ctrl_rdeu.rcocc31, ejercicio=ejercicio
        )
...
# --------------------------------------------------
def import_siif_last_rdeu012(
    ejercicio: str, import_df: ImportDataFrame
) -> pd.DataFrame:
...
# --------------------------------------------------
def rcocc31_in_rdue012(
    rdeu: pd.DataFrame, rcocc31: pd.DataFrame, ejercicio: str
) -> pd.DataFrame:
...
# --------------------------------------------------
def aju_not_in_rdue012(
    filter_rdeu: pd.DataFrame, rcocc31: pd.DataFrame, ejercicio: str
) -> pd.DataFrame:
    aju = rcocc31.loc[rcocc31["tipo_comprobante"].isin(["AJU"])]
